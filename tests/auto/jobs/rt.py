...
def run_regression_test(job_obj, pr_repo_loc):
    ...
    if job_obj.compiler == 'gnu':
        rt_command = [[f'export RT_COMPILER="{job_obj.compiler}" && cd tests '
                       '&& /bin/bash --login ./rt.ncar.sh -e -l rt_gnu.conf -p {job_obj.machine} -w {job_obj.workdir}',
                       pr_repo_loc]]
    elif job_obj.compiler == 'intel':
        rt_command = [[f'export RT_COMPILER="{job_obj.compiler}" && cd tests '
                       '&& /bin/bash --login ./rt.ncar.sh -e -p {job_obj.machine} -w {job_obj.workdir}', pr_repo_loc]]
